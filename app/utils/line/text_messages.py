...
สำหรับ พี่ม้าบิน Fertilizer Chatbot
...
def get_help_menu_text() -> str:
    ...
    return (
        "📋 เมนูช่วยเหลือ\n"
        "━━━━━━━━━━━━━━━\n\n"
        "พี่ม้าบินช่วยอะไรได้บ้าง:\n\n"
        "🌱 แนะนำสูตรปุ๋ย\n"
        "- บอกชนิดพืช + ระยะการเติบโต\n"
        "- พี่ม้าบินแนะนำสูตรปุ๋ยและอัตราการใช้ให้ครับ\n\n"
        "🌾 พืชที่รองรับ 6 ชนิด\n"
        "- นาข้าว, ข้าวโพด, อ้อย\n"
        "- มันสำปะหลัง, ปาล์มน้ำมัน, ยางพารา\n\n"
        "คำสั่งอื่นๆ:\n"
        "- พิมพ์ \"ดูผลิตภัณฑ์\" เพื่อดูสูตรปุ๋ย\n"
        "- พิมพ์ \"วิธีใช้งาน\" เพื่อดูคู่มือ\n"
        "- พิมพ์ \"ล้างความจำ\" เพื่อเริ่มใหม่\n\n"
        "💬 หรือพิมพ์ถามมาได้เลยนะครับ"
    )
...
def get_usage_guide_text() -> str:
    ...
    return (
        "📖 คู่มือการใช้งานพี่ม้าบิน\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "🌱 แนะนำสูตรปุ๋ย:\n"
        "- บอกชนิดพืชที่ปลูก\n"
        "- บอกระยะการเติบโต\n"
        "- พี่ม้าบินแนะนำสูตรปุ๋ยและอัตราการใช้ให้ครับ\n\n"
        "💬 ถามเรื่องปุ๋ยกับพี่ม้าบินได้นะครับ:\n"
        "- \"นาข้าว ระยะเร่งต้น ใช้ปุ๋ยอะไรดี\"\n"
        "- \"ข้าวโพดใช้สูตรปุ๋ยอะไร\"\n"
        "- \"อ้อยระยะย่างปล้อง แนะนำปุ๋ยหน่อย\"\n"
        "- \"ปาล์มน้ำมัน ใส่ปุ๋ยสูตรอะไร\"\n\n"
        "ลองพิมพ์ถามมาได้เลยครับ พี่ม้าบินพร้อมช่วยเลยนะครับ 😊"
    )
...
def get_product_catalog_text() -> str:
    ...
    return (
        ...
        "- ปาล์มน้ำมัน\n"
        "- ยางพารา\n\n"
        "💬 บอกชนิดพืชและระยะการเติบโต\n"
        "พี่ม้าบินแนะนำสูตรปุ๋ยให้ครับ"
    )
...
def get_other_plant_prompt_text() -> str:
    ...
    return (
        "🌿 กรุณาพิมพ์ชื่อพืชที่ต้องการวิเคราะห์ครับ\n\n"
...
def get_analyzing_text(with_info: bool = False) -> str:
    ...
    if with_info:
        return "🔬 กำลังวิเคราะห์รูปภาพพร้อมข้อมูลเพิ่มเติม... กรุณารอสักครู่ครับ"
    return "🔬 กำลังวิเคราะห์รูปภาพ... กรุณารอสักครู่ครับ"
...
def format_product_list_text(products: List[Dict]) -> str:
    """Format รายการปุ๋ยแนะนำเป็น text"""
    if not products:
        return "ไม่พบสูตรปุ๋ยที่เหมาะสม"
    ...
    for idx, p in enumerate(products[:5], 1):
        formula = p.get("fertilizer_formula", "ไม่ระบุ")
        crop = p.get("crop", "-")
        stage = p.get("growth_stage", "-")
        rate = p.get("usage_rate", "-")
        nutrients = p.get("primary_nutrients", "-")
        benefits = p.get("benefits", "-")

        lines.append("")
        lines.append(f"{'─'*20}")
        lines.append(f"{idx}. สูตร {formula}")
        lines.append(f"   พืช: {crop}")
        lines.append(f"   ระยะ: {stage}")
        lines.append(f"   ธาตุอาหารหลัก: {nutrients}")
        lines.append(f"   อัตราใช้: {rate}")
        lines.append(f"   ประโยชน์: {benefits}")

    lines.append("")
    lines.append("━━━━━━━━━━━━━━━")
    lines.append("⚠️ ปรับอัตรา/ปริมาณตามสภาพดินและพืชจริงก่อนใช้ทุกครั้ง")
...
def get_continue_or_cancel_text(current_task: str) -> str:
    ...
    return (
        f"⚠️ มีงานค้างอยู่\n\n"
        f"คุณกำลัง{current_task}อยู่ครับ\n\n"
