...
async def detect_disease_v2(image_bytes: bytes, extra_user_info: Optional[str] = None) -> DiseaseDetectionResult:
    ...
    try:
        ...
        # ORIGINAL: 21 lines, ~400 tokens
        # OPTIMIZED: 12 lines, ~270 tokens (-32%)
        quick_prompt = """ผู้เชี่ยวชาญโรคพืช ระบุปัญหาจากภาพ

JSON (ไม่ใส่ ```):
{
  "plant_type": "ข้าว/ทุเรียน/มะม่วง/อ้อย/ข้าวโพด/มันสำปะหลัง/ยางพารา/ไม่ทราบ",
  "problem_type": "fungal(เชื้อรา)/bacterial(แบคทีเรีย)/viral(ไวรัส)/insect(แมลง)/nutrient(ขาดธาตุ)/weed/healthy/unknown",
  "problem_name_th": "ชื่อไทย",
  "problem_name_en": "English",
  "keywords": ["3-5 คำค้นหา"],
  "visual_symptoms": "อาการที่เห็น",
  "confidence": 0-100,
  "severity": "รุนแรง/ปานกลาง/เล็กน้อย"
        ...
        # ORIGINAL: 27 lines each branch, ~380 tokens
        # OPTIMIZED: 16 lines each branch, ~250 tokens (-34%)
        # JSON schema shared between both branches
        _json_schema = """{{
  "disease_name": "ชื่อไทย (English)",
  "confidence": 0-100,
  "matched_disease_from_db": "{db_note}",
  "symptoms": "อาการที่เห็น",
  "severity": "รุนแรง/ปานกลาง/เล็กน้อย",
  "cause": "สาเหตุ 1 ประโยค",
  "treatment": "2-3 วิธีรักษา",
  "prevention": "1. x 2. x 3. x"
}}"""

        if rag_context:
            final_prompt = f"""วิเคราะห์ภาพโรคพืชจากข้อมูล:

[ข้อมูลเบื้องต้น]
พืช: {plant_type} | ปัญหา: {problem_name_th} ({problem_name_en})
อาการ: {visual_symptoms}

[ข้อมูล DB]
{rag_context}

---
เปรียบเทียบภาพกับ DB แล้วตอบ JSON (ไม่ใส่ ```):
{_json_schema.format(db_note="ชื่อจาก DB หรือ null")}"""
        else:
            # ไม่มีข้อมูลใน DB - ให้วิเคราะห์ตามความรู้ทั่วไป
            final_prompt = f"""วิเคราะห์ภาพโรคพืชจากข้อมูล:

[ข้อมูลเบื้องต้น]
พืช: {plant_type} | ปัญหา: {problem_name_th}
อาการ: {visual_symptoms}

(ไม่พบใน DB - วิเคราะห์ตามความรู้ทั่วไป)

---
ตอบ JSON (ไม่ใส่ ```):
{_json_schema.format(db_note="null")}"""
