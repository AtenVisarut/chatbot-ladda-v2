...
async def handle_natural_conversation(user_id: str, message: str) -> str:
    ...
    try:
        ...
        # Priority 1: Agriculture Question (พืช/โรคพืช/การเกษตร) → ค้นหาจาก diseases table ก่อน
        if is_agriculture_question(message) or keywords["pests"] or keywords["crops"]:
            logger.info(f"🌾 Routing to agriculture Q&A (pests={keywords['pests']}, crops={keywords['crops']})")
            answer = await answer_agriculture_question(message, context)

            # Add assistant response to memory
            await add_to_memory(user_id, "assistant", answer)
            return answer

        # Priority 2: Fertilizer/Product Knowledge Base
        if keywords.get("is_fertilizer_query"):
            logger.info(f"Routing to knowledge base (fertilizers={keywords.get('fertilizers', [])})")
        ...
        # Priority 3: Product Recommendation (ถ้าเป็น product query แต่ไม่มี pests/crops)
