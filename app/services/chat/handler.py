...
from app.prompts import (
    GENERAL_CHAT_PROMPT, ERROR_GENERIC, ERROR_AI_UNAVAILABLE, GREETINGS, GREETING_KEYWORDS,
    build_legacy_qa_prompt, build_legacy_qa_system_msg, build_usage_prompt, USAGE_SYSTEM_MSG,
)
...
AGRICULTURE_KEYWORDS = [
    ...
    # กลุ่มสาร/เทคนิค
    "irac", "frac", "hrac", "rac", "กลุ่มสาร", "กลุ่มเคมี", "สารออกฤทธิ์",
]
...
NUTRIENT_KEYWORDS = [
    ...
    # การบำรุง
    # ห้าม: "ติดดอก","ติดผล" เดี่ยวๆ เพราะเป็น stage — ใช้ "ไม่ติดดอก"/"ไม่ติดผล"
    # (อาการขาดธาตุ) + "บำรุงดอก"/"บำรุงผล" (nutrient intent) แทน
    "ขยายผล", "บำรุงดอก", "บำรุงผล", "บำรุงต้น",
    "เร่งดอก", "เร่งผล", "สะสมอาหาร", "เพิ่มผลผลิต",
    # Nutrient intent ที่อ้างถึงระยะข้าว — ต้องเป็นคำประสม "เร่ง+ระยะ"
    # ห้ามใส่ระยะเดี่ยวๆ ("แตกกอ","ออกรวง","ตั้งท้อง") เพราะนั่นคือ phenology
    # ไม่ใช่ nutrient — จะชน Stage -1 clarification reply
    "เร่งแตกกอ", "เร่งออกรวง", "เร่งตั้งท้อง",
    # ธาตุเฉพาะ
...
FARMER_SLANG_MAP = {
    ...
    "ข้าวดีด": {"hint": "ข้าวดีด/ข้าวตีด — วัชพืชในนาข้าว (ข้าวป่า/ข้าวหาง)", "problem_type": "weed", "search_terms": ["ข้าวดีด", "ข้าวตีด", "วัชพืชข้าว", "กำจัดข้าวดีด"]},
    "ข้าวตีด": {"hint": "ข้าวตีด/ข้าวดีด — วัชพืชในนาข้าว (ข้าวป่า/ข้าวหาง)", "problem_type": "weed", "search_terms": ["ข้าวตีด", "ข้าวดีด", "วัชพืชข้าว", "กำจัดข้าวตีด"]},
}
...
async def answer_qa_with_vector_search(question: str, context: str = "") -> str:
    ...
    try:
        ...
        # สร้าง prompt จาก centralized templates (prompts.py)
        prompt = build_legacy_qa_prompt(question, context, combined_context)
        ...
        response = await openai_client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": build_legacy_qa_system_msg(allowed_products_str)},
...
async def answer_usage_question(user_id: str, message: str, context: str = "") -> str:
    ...
    try:
        ...
        prompt = build_usage_prompt(message, products_text, context)
        ...
        response = await openai_client.chat.completions.create(
            model=LLM_MODEL_GENERAL_CHAT,
            messages=[
                {"role": "system", "content": USAGE_SYSTEM_MSG},
                {"role": "user", "content": prompt}
            ],
            max_completion_tokens=LLM_TOKENS_HANDLER_RAG,
...
def _check_unsupported_question(message: str) -> str | None:
    ...
    if any(p in msg for p in _PHI_PATTERNS):
        return (
            "ขออภัยค่ะ ข้อมูลระยะหยุดยาก่อนเก็บเกี่ยว (PHI) ยังไม่ครบถ้วนในระบบของน้องลัดดาในขณะนี้ค่ะ 😊\n"
