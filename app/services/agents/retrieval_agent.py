...
class RetrievalAgent:
    ...
    async def _search_knowledge(
        ...
        """Search knowledge table with vector similarity"""
        ...
        try:
            ...
            result = self.supabase.rpc(
                'match_knowledge',
                {
                    ...
                    'match_count': top_k * 2  # Get more for filtering
                }
            ).execute()
            ...
            for item in result.data:
                ...
                # Apply entity-based filtering if product name specified
                if query_analysis.entities.get('product_name'):
                    product_in_query = query_analysis.entities['product_name'].lower()
                    doc_product = (doc.metadata.get('product_name') or '').lower()
                    doc_title = doc.title.lower()
                    if product_in_query not in doc_product and product_in_query not in doc_title:
                        continue  # Skip non-matching products

                docs.append(doc)

            logger.info(f"    Knowledge search: {len(docs)} docs for '{query[:30]}...'")
            return docs
